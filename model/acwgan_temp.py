...
class ResBlockDiscriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("self.bypass(x).shape: ", self.bypass(x).shape)
        return self.model(x) + self.bypass(x)
...
class Generator(nn.Module):
    def __init__(self, z_dim, c_dim):
        ...
        conv_intermediate  = nn.Conv2d(GEN_SIZE*4 + c_dim, GEN_SIZE*4, 3, 2, padding=1)
        conv_intermediate2 = nn.Conv2d(GEN_SIZE*4, GEN_SIZE * 4, 3, 2, padding = 0)

        nn.init.xavier_uniform_(conv_intermediate.weight.data, 1.)
    ...
    def forward(self, z, c):
        ...
        hc = torch.cat([h, c.type(torch.float32)], dim=1)
...
class Discriminator(nn.Module):
    def __init__(self, c_dim):
        ...
        self.classify = nn.Linear(DISC_SIZE*8, c_dim)
        nn.init.xavier_uniform_(self.classify.weight.data, 1.)
    def forward(self, x):
        h = self.model(x).view(-1,DISC_SIZE*8)
...
class Discriminator_CNN(nn.Module):
    ...
    def __init__(self, c_dim=5):
        ...
        repeat_num=6
        layers = []
        layers.append(nn.Conv2d(1, conv_dim, kernel_size=3, stride=2, padding=1))
